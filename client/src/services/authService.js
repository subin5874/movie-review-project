...
const useCheckAccessToken = () => {
  ...
  useEffect(() => {
    const checkAccessToken = async (token) => {
      ...
      try {
        ...
        if (err.response && err.response.status == 401) {
          console.log(
            'ToAccessToken이 유효하지 않습니다. RefreshToken을 검증합니다.'
          );
          try {
            ...
            if (response2 && response2.status == 200) {
              console.log('RefreshToken이이 유효하여 재발급을 진행합니다.');
            ...
            if (err.response2 && err.response2.status == 401) {
    ...
    //Token 검증 테스트를 위해 60초마다 동작하도록 함
    const intervalId = setInterval(checkAccessToken, 60 * 1000);
